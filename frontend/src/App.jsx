    ...
    return (
        <div className="container">
            ...
            <div className="nonsticky-scroll-wrapper">
                ...
                <CardScrollRow
                    cards={ownedCards
                        .map(name => scoredCards.find(c => c.name === name) || cards.find(c => c.name === name))
                        .filter(Boolean)}
                    toggleOwned={name => setOwnedCards(prev => prev.filter(c => c !== name))}
                    owned={true}
                />
            </div>
            ...
            <div className="sticky-scroll-wrapper">
                ...
                <CardScrollRow
                    ...
                    toggleOwned={name => setOwnedCards(prev => [...prev, name])}
                    owned={false}
                />
            </div>
            <CategoryTabs
...
function CardScrollRow({ cards, toggleOwned, owned }) {
    return (
        <div className="scroll-row">
            {cards.map(card => {
                const additionalValue = card.score ?? 0;
                    ...
                    <div className="card-box" key={card.name}>
                        ...
                        <div className="card-content">
                            ...
                            <div className="card-info">
                                {"score" in card && (
                                    <>
                                        <p><strong>{owned ? "Value" : "Value Added"}:</strong> {additionalValue.toFixed(2)}</p>
...
function CategoryTabs({ categories, answers, setAnswers }) {
    const [currentTab, setCurrentTab] = useState(0);
            ...
            <div className="sticky-tab-wrapper">
                <div className="question-tabs">
                    {categories.map((cat, index) => (
                        <div
                            key={index}
                            className={`question-tab ${index === currentTab ? "active" : ""}`}
